        ...
        <div class="w-full bg-gray rounded-lg shadow-md">
            <div class="flex justify-between rounded-t-lg items-center bg-primary py-5 px-4">
                ...
                <form action="{{ route('store') }}" method="POST" class="flex w-full">
                    @csrf
                    <input name="content" type="text" class="py-3 px-6 bg-white transition border-2 border-secondary rounded w-full placeholder-opacity-50 placeholder-black focus:outline-none focus:border-secondary" placeholder="{{ $errors->any() ? $errors->first('content') : "Add Task" }}">
